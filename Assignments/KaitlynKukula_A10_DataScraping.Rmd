...
water_dataframe <- data.frame(
  ...
  date = dmy(paste0("01-",water_dataframe$month,"-",water_dataframe$year)),
...
ggplot(water_dataframe, aes(x = month(date), y = mgd_monthly)) +
  ...
  labs(x = NULL,
       y = "Average Daily Uses [MGD]",
       title = "Monthly Average Daily Use of Water in Durham, 2024")
...
# Get the URL 
   lwsp_fcn_url <- ifelse(
    year_input == '2024' & pwsid_input == '03-32-010',
    paste0('https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=',
           pwsid_input,'&year=',year_input),
    'https://www.ncwater.org'
    )
   
# Fetch the website
    lwsp_fcn_website <- read_html(lwsp_fcn_url)
    print(lwsp_fcn_website)
    
# Scrape the data
    water_system_fcn <- lwsp_fcn_url %>% html_nodes('div+ table tr:nth-child(1) td:nth-child(2)') %>% html_text()
    pwsid_fcn <- lwsp_fcn_url %>% html_nodes('td tr:nth-child(1) td:nth-child(5)') %>% html_text()
    owner_fcn <- lwsp_fcn_url %>% html_nodes('div+ table tr:nth-child(2) td:nth-child(4)') %>% html_text()
    mgd_monthly_fcn <- lwsp_fcn_url %>% html_nodes('th~ td+ td') %>% html_text()
}
