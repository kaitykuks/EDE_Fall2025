...
mapview(huc8_sf, col.regions = 'gray') +
  mapview(huc8_sf_neuse, col.regions = 'yellow', color = 'blue')

ggplot() +
  geom_sf(data = huc8_sf) +
  geom_sf(data = 
            huc8_sf_neuse, fill = 'yellow', color = 'blue')
...
huc8_wind <- read_csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv')
huc8_wind$HUC_12 <- as.factor(huc8_wind$HUC_12)
huc8_wind_final <- huc8_wind %>%
...
the_buffer <- theSite_sfc %>% st_buffer(theSite_sfc_transformed,30000)
...
# Filter the counties dataset using the boolean mask
setCounties2 <- counties_sf_utm[resultmask,]
