...
str(GasPlants$geometry)
  # XY Coordinate that can tell you location.

#Explore as a spatial dataframe...
#View using sf plot
plot(GasPlants['status'],pch=16) #Specify the column you want, color by is "status", 

#View using ggplot
ggplot(data=GasPlants) + geom_sf(aes(color=status)) #geom_sf for spatial analysis, many commands are the same, axis are long-lat
...
EPAair.sites <- EPAair %>% 
  ...
  summarize(
    mean_Ozone = mean(Ozone, na.rm=T), #What does "na.rm=T" do? Is it necessary? - Run with data that exists and ignore ND entries.
...
mapview(EPAair.sf, cex='mean_PM25') #cex allows you to resize the points based on a variable
...
mapview(sfg_GraingerHall) # Struggles to plot this because it knows what the coordinates are, its doesnt know where the reference is (missing spatial reference)

#Upgrade to a simple feature geometry column by adding a crs 
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) # Specified CRS
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617) # Reprojected to UTM
GasPlants.utm <- st_transform(GasPlants,32617) # Reprojected to UTM
...
distance_matrix <- st_distance(EPAair.sf.utm, GasPlants.utm, pairwise=T)
 # st_distance inputs two dataframes and calculates pairwise distance between points
 # Use this to find site distance from a point or something
dim(distance_matrix)
