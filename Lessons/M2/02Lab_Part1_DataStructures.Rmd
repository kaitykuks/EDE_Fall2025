...
Discuss and navigate different data types in R
 - Vectors: sequence of numbers that only contains one data type
 - Matrices
 - Arrays
 - Data frames
...
# Returns the maximum value in the vector (does thi only work for numberic forms?)
max(vector1)

#Creates another vector of the values 10 thru 15, defaults to a delineation of 1
vector4 <- c(10:15)

# Stitches together vectors - in this form will not be saved to a variable
c(vector1, vector4)

# Multiplies the two vectors on an index-by-index basis
vector1 * vector4

# Multiplies each vector entry by a constant
vector1 * 10

# Converts each entry into uppercase
toupper(vector2) 

# Inverts each entry of a logical vector
!vector3
...
temps_f <- c(200,81,81,83,83)

#What is the average of these temperatures 
temps_mean <- mean(temps_f)

#Compute the difference between each temperature and the average calculated above
temps_diff <- temps_f - temps_mean  #You don't have to use c here because temps_f is a vector, so it will automatically be a vector.
...
matrix1
#matrix1 has 5 rows and 4 columns and populates by columns

matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #
matrix2
#matrix2 has 5 rows and 4 columns and populates by row first
...
matrix3 <- matrix(
  ...
  ) # dimnames gives the row and columns names
...
matrix1[4, 1] # Returns row 4, column 1

matrix1[4,  ] # Returns row 4

matrix1[ , 3] # Returns column 3

matrix1[c(12, 14)] # Returns the 12th and 14th entry by columns

matrix1[c(12:14)] # Returns entries 12 through 14

matrix1[2:4, 1:3] # Returns the row entries 2-4 and column entries 1-3 (clips data)
...
```{r Creating Lists}
# Lists
# Any data type can go into a list.
# Ket and Variable pairs
# Creating Lists
list1 <- list(
  name = "Maria", 
  mynumbers = vector1, 
...
dataframe1$month <- c('Jan','Feb', 'Mar', 'Apr') # $ is used to add a column
