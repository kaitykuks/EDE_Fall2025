...
Data = model + residual or error term

Data: continuous variable y
Model: one or more variables, continuous or categorica

### Choosing a model from your data: A "cheat sheet"

**One-way ANOVA (Analysis of Variance):** Continuous response, one categorical explanatory variable with more than two categories you want to compare to.
...
**Single Linear Regression** Continuous response, one continuous (independent - y) explanatory variable.
- Technique for fitting a line to a set of data points (scatter plot)
- Standard error: a measure of the scatter of points around a regression line
  - smaller = better
- Correlation coefficient (r): measure of the strength and direction of relationship between two variables
  - Ranges from -1 to 1 (no correlation = 0)
  - r^2 = measure of the percentage of variability in the values of y that is explained by the independent variable ( 0 to 1)
...
Analyze evidence provided by data and make decisions
- H_o: null hypoithesis
- H_a: alternative hypothesis (opposite of null hypothesis)

Procedure:
1. State the hypothesis and identify the claim
2. Find the critical value(s) from the appropriate table
3. Claim the test value
...
- indicates if a relationship is statistically significant
...
irradiance.regression <- lm(
  data = PeterPaul.chem.nutrients, # use this format instead of the $
  irradianceWater ~ depth)
summary(irradiance.regression) 
  # Results: 
    # Residuals: min, med, max
    # Coefficients provided for the equation (intercept, slope)
    # Residual std errors, multiple R-squared (depth explains about 30% of the variability which is actually pretty low), F-statistic (p-value-meaningful regression performed)

# Correlation
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth)
  # Results:
    # Negative correlation, there is a strong correlation between depth decreasing and light level decreasing
```
Question: How would you report the results of this test (overall findings and report of statistical output)?

>  You could return the regression line equation, R2 value, and p-value
...
1. **Residuals vs. Fitted.** The value predicted by the line of best fit is the fitted value, and the residual is the distance of that actual value from the predicted value. By definition, there will be a balance of positive and negative residuals if this fit is good. Watch for drastic asymmetry from side to side or a marked departure from zero for the red line - these are signs of a poor model fit.

2. **Normal Q-Q.** Quantile-Quantile plot. Compare the values to a normal distribution. The points should fall close to the 1:1 line. We often see departures from 1:1 at the high and low ends of the dataset, which could be outliers. 
...
#par(mfrow = c(2,2), mar=c(4,4,4,4)) # Dividing my plot screen into four cells (creating a grid)
plot(irradiance.regression)
par(mfrow = c(1,1)) # Plot just the regressions one-by-one
```
...
```{r, fig.height = 3, fig.width = 4}
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, # Filtering out outliers
...
summary(irradiance.regression2)
 # Resulting residual error is lower than before, which is good.
...
It is possible, and often useful, to consider multiple continuous explanatory variables (more than one x variable) at a time in a linear regression. For example, total phosphorus concentration in Paul Lake (the unfertilized lake) could be dependent on depth and dissolved oxygen concentration: 

``` {r, fig.height = 3, fig.width = 4}
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), #subset(dataset, variable == choice)
                   tp_ug ~ depth + dissolvedOxygen) # y ~ x1 + x2
summary(TPregression)

 # Results:
    # Depth is positively correlated, DO is negatively correlated
    # Small p-values are small, so the differences are not zero
    # Residual is 6.405
    # R2 is about 30% (only 30% of variably of P concentration is due to DO and depth)
...
corrplot(PeterPaulCorr, method = "ellipse") # Use this to see what variables are correlated
...
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) # Use this to test which variables are explanatory

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC)
  # Results:
    # Starting point is the first regression with the four variables we specified

TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel)
  # Results:
    # R2 value is worse for this actually  
