...
To reject null hypothesis, at least one pair of means is not the same as the others. To resolve this, you need to run post hoc tests.
...
summary(Litter.Totals)
  #Results:
    #descritive statistics, like min, med, mean, max

#check all value of plotID
summary(Litter.Totals$plotID)
  # Shows that some sites have more observations that anothers, but we can still run an ANOVA
...
# shapiro.test() performs the Shapiro-Wilk test of the null that data follows a normal distribution
  # Tests that the data follows a normal distribution

shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_040"]) # p-value < 0.005 (reject null)
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_041"])
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_046"]) # Accept null hypothesis (follows normal distrib.)
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_057"]) # Accept null hypothesis (follows normal distrib.)
...
qqnorm(Litter.Totals$dryMass); qqline(Litter.Totals$dryMass)
  # Shows that the data does no follow a normal distribution
...
bartlett.test(Litter.Totals$dryMass ~ Litter.Totals$plotID)
  #Results: reject null (small p value) i.e. variances are not equal

# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova)

#results: reject null hypothesis i.e. difference between a pair of group means its statistically significant
# Sum sq is the average??? derivation from the mean
# F-value is the mean sq plot ID / mean sq residuals
# Doesn't tell which are different or the same

# Format ANOVA as lm (more dependent on detail)
...
# Used to compare all possible group pairings
TukeyHSD(Litter.Totals.anova)

# Extract groupings for pairwise relationships that have similar means
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE) # (snove object, column, group is to group levels that have the same mean)
Litter.Totals.groups
  # Results
    # Possible treatment groups - groups with the same letters are not significantly different
...
> Reorder the sites based on groups
...
For example: What if we wanted to know how litter masses (response variable) varied across functional groups (explanatory one) and land cover class (explanatory two)?
```{r}
# Format as aov
...
TukeyHSD(Litter.anova.2way)
  # Possible combinations of the functional groups
  # Effects of func group and land cover can be dependent on eachother, so you might need to run interaction effects 
...
Litter.interaction <- with(Litter, interaction(functionalGroup, nlcdClass))
  # with(dataset, interaction(two classes))
  # with evaluates the expression in a local environment contructed from dataset
  # useful for simplifying calls to modeling functions
