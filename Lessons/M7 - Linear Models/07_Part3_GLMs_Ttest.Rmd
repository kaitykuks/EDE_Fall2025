...
The null hypothesis must have the equality (mean of the group is equal to a specific value).
...
> Answer: 
H0: mu >= 50. Equality is ALWAYS on the null hypothesis
Ha: mu < 50
...
# Format as a GLM
O3.onesample2 <- lm(Ozone ~ 1, EPAair) # ~1 returns the intercept coefficient, this is what you compare the mean to.
O3.onesample2
```
...
> The resulting intercept is 40.88, which is not greater than or equal to 50. This means that we need to reject the null hypothesis that air quality is above the threshold for poor AQI index.
...
O3.twosample
  # Results:
    #p value < 0.05 so reject null hypothesis (true difference in means between 2018 and 2019 are not == 0)

# Format as a GLM
# Ask is year a predictor of ozone levels?
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Year) #Can also use lm(Ozone ~ Year, EPAair)
summary(O3.twosample2)
   # Results
    # p vale < 0.05 so the coefficient 0.8452 is different from 0 so reject null hypothesis, so year is a predictor of ozone
