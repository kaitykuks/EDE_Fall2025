...
Month <- as.data.frame(seq.Date(from = as.Date("1901/01/01"),
                                 to = as.Date("2016/12/01"),
                                 by = "month"))
colnames(Month) <- 'Date'
BR_complete <- left_join(Month, BR_complete)
...
BR_rainfall_plot <- ggplot(BR_complete, aes(x = Date, y = Rainfall)) +
  geom_line() +
  geom_smooth(method = 'lm')
BR_rainfall_plot  
  # Rainfall is defined by seasonality, although intensity may vary year by year.

BR_temperature_plot <- ggplot(BR_complete, aes(x = Date, y = Temperature_C)) +
  geom_line() +
  geom_smooth()
BR_temperature_plot 
  # temperature is increasing over time and there is seasonality.
...
BR_ts_rainfall <- ts(data = BR_complete$Rainfall,
                    start = c(f_year, f_month),
                    frequency = 12)

BR_ts_temp <- ts(data = BR_complete$Temperature_C,
                    start = c(f_year, f_month),
                    frequency = 12)
...
BR_stl_rainfall <- stl(BR_ts_rainfall, s.window = "periodic")
plot(BR_stl_rainfall)
BR_rainfall_comps <- as.data.frame(BR_stl_rainfall$time.series[,1:3])

BR_stl_temp <- stl(BR_ts_temp, s.window = "periodic")
plot(BR_stl_temp)
BR_temp_comps <- as.data.frame(BR_stl_temp$time.series[,1:3])


# Exercise 10: Visualize how the trend maps onto the data for both series

rainfall_trend <- trend::smk.test(BR_ts_rainfall)

rainfall_trend_plot <- ggplot(BR_complete) +
  geom_line(aes(x = Date, y = Rainfall)) +
  geom_line(rainfall_trend_plot)
rainfall_trend_plot
