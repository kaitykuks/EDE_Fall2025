...
> Add notes here: These are common markers that can be used to understand the health and conditions of a lake.
...
# matrix filtering: filter rows that have a 0 in column depth using matrix notation
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] 

# dplyr filtering: filter rows that have a 0 or value <0.25 in the depth column using dplyr natations
...
# Determine how many lakes are included in the dataset
unique(NTL.phys.data$lakename) 

# Create a new dataframe with these two lakes
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")

# Create a new dataframe with all lakes except these
...
# Create a new dataframe with all lakes listed
...
NTL.phys.data.Exercise1 <- filter(NTL.phys.data,
...
arrange(NTL.phys.data.PeterPaul1, desc(sampledate))
...
NTL.phys.data.processed <- 
  ...
  select(lakeid:daynum, month, sampledate:temperature_C) %>%
