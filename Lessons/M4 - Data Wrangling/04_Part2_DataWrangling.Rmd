...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% # Dataset you are working on
  mutate(month = month(sampledate)) %>% # Create a column with the month
  select(lakeid:daynum, month, sampledate:comments) %>% # Reorder to columns so month is between day and sample date
  drop_na(depth) # Delete any values that do not have depth entries
...
Litter <- LitterFiles %>%
  plyr::ldply(read.csv) # Access LitterFiles list of names, then read them all as .csv's
```
