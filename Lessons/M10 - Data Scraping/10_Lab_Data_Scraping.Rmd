...
#Get the data, which starts on line 27
gage_data_elwha <- read.table(
  theURL_elwha,
  skip = 27, #Skip non-data lines
  header=TRUE,
  sep='\t',
  stringsAsFactors = T) 

#Update the column headers
colnames(gage_data_elwha) = c("agency_cd", "site_no", "datetime", "tz_cd","gage_ht_ft", "150691_00060_cd")

#Tidy the data
gage_data_elwha <- gage_data_elwha %>% 
  select(datetime, gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(gage_data_elwha,aes(x=datetime, y=gage_ht_ft)) + 
  geom_line() +
  labs(x = "Date", y = "Gage height [ft]",
       title = "Elwha River Gage Height 2006 - 2014")
```
...
energy_df <- data.frame(
  ...
  net_generation = gsub(',','',net_generation),
