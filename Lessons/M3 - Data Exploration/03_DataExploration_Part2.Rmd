...
USGS.flow.data <- read.csv(
  ...
  stringsAsFactors = TRUE,  # Imports all string variables as factors
...
# Generate bins so that each has a width of 10 (10 units of discharge)
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# Generate bins so that there is a total of 20 bins (evenly distributed bins to total 20)
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# Use binwidth of 10, but truncate any discharge means over 500
...
ggplot(USGS.flow.data) +
  ...
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2, color = "blue") +
...
> Answer: This depends on if you have discrete or continuous data, are interested in how tight distributions are, how much you have in each category, change over time, etc. 

## Ending discussion
What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: This datasets provides information on trends in the hydrology over almost a century. Difference types of charts showed concepts such as how these trends changed over time or what the most common stream conditions are. 


How can multiple options for data exploration inform our understanding of our data?

> Answer: Different options can show differences in discrete and continuous relationships, as well as provide more information on statistics (eg boxplot) vs relationships (eg scatterplot).
...
> Answer: Many of the trends observed seem logical to me, such as the relationship between discharge and gage height. 
