...
here('NTL-LTER_Lake_ChemistryPhysics_Raw.csv')
...
EPA_filenames <- dir(here('Data', 'Raw'), 
                     pattern = ("^EPAair_O3*.*csv"), # * after _O3 means that you want to access any files with any value that                                                        follows the 3. (ie EPAair_O3_Dec23) 
                     full.names = T) # Instead of just listing the file name, list the full directory pathway
print(EPA_filenames)

EPAdata <- EPA_filenames %>% # Pipe shortcut is shift+control+M
  map(read.csv) %>% # Applies read.csv to everything the EPA_filenames and produces a list
  reduce(rbind) # Make one dataframe where all the rows from the second file are added
    # after the rows from the first file -- could also use bind_rows()
      # reduce() 
      # rbind() actually joins the vectors
```
...
fixed_date_obj_past <- fix.early.dates(date_obj_past,cutoff=25) 
